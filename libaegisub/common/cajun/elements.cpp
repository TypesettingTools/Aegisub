...
template <typename T> struct CastVisitor final : public CastVisitorBase {
	T* element = nullptr;
...
namespace {
template <typename ElementTypeT> class Imp_T final : public UnknownElement::Imp {
...
#define DEFINE_UE_TYPE(Type) \
	...
	UnknownElement::operator Type const&() const { \
		return CastTo<Type>(); \
	} \
	UnknownElement::operator Type&() { \
		return CastTo<Type>(); \
	}
...
template <typename ElementTypeT> ElementTypeT const& UnknownElement::CastTo() const {
	CastVisitor<ElementTypeT> castVisitor;
	const_cast<UnknownElement*>(this)->Accept(castVisitor);
	if(!castVisitor.element) throw Exception("Bad cast");
	return *castVisitor.element;
}

template <typename ElementTypeT> ElementTypeT& UnknownElement::CastTo() {
	CastVisitor<ElementTypeT> castVisitor;
	Accept(castVisitor);
