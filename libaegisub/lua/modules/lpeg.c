...
static int sizei(const Instruction* i) {
	switch((Opcode)i->i.code) {
		case ISet:
		case ISpan: return CHARSETINSTSIZE;
...
static void printcapkind(int kind) {
	const char* const modes[] = { "close",        "position", "constant", "backref", "argument",
		                          ...
		                          "substitution", "fold",     "runtime",  "group" };
	printf("%s", modes[kind]);
}
...
static int runtimecap(lua_State* L, Capture* close, Capture* ocap, const char* o, const char* s,
                      int ptop);

static Capture* doublecap(lua_State* L, Capture* cap, int captop, int ptop) {
	Capture* newc;
	if(captop >= INT_MAX / ((int)sizeof(Capture) * 2)) luaL_error(L, "too many captures");
...
static Stack* doublestack(lua_State* L, Stack** stacklimit, int ptop) {
	...
	lua_pop(L, 1);
	if(n >= max) luaL_error(L, "too many pending calls/choices");
	newn = 2 * n;
...
static const char* match(lua_State* L, const char* o, const char* s, const char* e, Instruction* op,
	...
	for(;;) {
#if defined(DEBUG)
		printf("s: |%s| stck: %ld c: %d  ", s, stack - getstackbase(L, ptop), captop);
		printinst(op, p);
#endif
		switch((Opcode)p->i.code) {
			case IEnd: {
...
*/
static int verify(lua_State* L, Instruction* op, const Instruction* p, Instruction* e, int postable,
                  int rule) {
	...
	while(p != e) {
		switch((Opcode)p->i.code) {
			...
			case IChoice: {
				if(backtop >= MAXBACKVER) return luaL_error(L, "too many pending calls/choices");
			...
			fail : { /* pattern failed: try to backtrack */
				do {
					if(backtop-- == 0) return 1; /* no more backtracking */
...
static void checkrule(lua_State* L, Instruction* op, int from, int to, int postable, int rule) {
	int i;
...
static void optimizejumps(Instruction* p) {
	int i;
	for(i = 0; p[i].i.code != IEnd; i += sizei(p + i)) {
		if(isjmp(p + i)) p[i].i.offset = target(p, dest(p, i)) - i;
	}
}
...
static int addpatt(lua_State* L, Instruction* p, int p1idx) {
	...
	if(corr != 0) {
		...
		for(px = p; px < p + sz; px += sizei(px)) {
			if(isfenvoff(px) && px->i.offset != 0) px->i.offset += corr;
		}
	}
	return sz;
}
...
static Instruction* newpatt(lua_State* L, size_t n) {
	Instruction* p;
	if(n >= MAXPATTSIZE - 1) luaL_error(L, "pattern too big");
...
static void fillcharset(Instruction* p, Charset cs) {
	switch(p[0].i.code) {
		...
		default: { /* any char may start unhandled instructions */ loopset(i, cs[i] = 0xff); break;
		}
	}
}
...
static int exclusive(CharsetTag* c1, CharsetTag* c2) {
	if(c1->tag == NOINFO || c2->tag == NOINFO)
		...
		return exclusiveset(c1->cs, c2->cs);
}
...
static Instruction* fix_l(lua_State* L, int t) {
	...
	while(lua_next(L, t) != 0) {
		...
		if(!testpattern(L, -1)) luaL_error(L, "%s is not a pattern", val2str(L, -2));
		l = pattsize(L, -1) + 1; /* space for pattern + ret */
		if(totalsize >= MAXPATTSIZE - l) luaL_error(L, "grammar too large");
...
static int pattbehind(lua_State* L) {
	...
	if(!nocalls(p1)) luaL_error(L, "lookbehind pattern cannot contain non terminals");
	if(isfail(p1) || issucc(p1))
		lua_pushvalue(L, 1); /* <fail == fail; <true == true */
...
static int firstpart(Instruction* p, int l) {
	if(istest(p)) {
		...
		if((p[e].i.code == IJmp || p[e].i.code == ICommit) && e + p[e].i.offset == l) return e + 1;
	} else if(p[0].i.code == IChoice) {
		int e = p[0].i.offset - 1;
		if(p[e].i.code == ICommit && e + p[e].i.offset == l) return e + 1;
	}
	return 0;
}

static Instruction* auxnew(lua_State* L, Instruction** op, int* size, int extra) {
...
static Instruction* basicUnion(lua_State* L, Instruction* p1, int l1, int l2, int* size,
                               CharsetTag* st2) {
	Instruction* op;
	CharsetTag st1;
	tocharset(p1, &st1);
...
static Instruction* separateparts(lua_State* L, Instruction* p1, int l1, int l2, int* size,
                                  CharsetTag* st2) {
...
static Instruction* repeats(lua_State* L, Instruction* p1, int l1, int n) {
	...
	Instruction* p = op;
	if(!verify(L, p1, p1, p1 + l1, 0, 0)) luaL_error(L, "loop body may accept empty string");
	for(i = 0; i < n; i++) {
		p += addpatt(L, p, 1);
	}
	setinst(p++, IChoice, 1 + l1 + 1);
...
static int star_l(lua_State* L) {
	...
	if(n >= 0) {
		CharsetTag st;
		Instruction* op;
		if(tocharset(p1, &st) == ISCHARSET) return repeatcharset(L, st.cs, l1, n);
		if(isheadfail(p1))
...
static int tcapture_l(lua_State* L) {
	return capture_aux(L, Ctable, 0);
}
static int capsubst_l(lua_State* L) {
...
#define updatecache(cs, v) \
	{ \
		if((v) != (cs)->valuecached) updatecache_(cs, v); \
	}
...
static int foldcap(CapState* cs) {
	...
	if(n > 1) lua_pop(L, n - 1); /* leave only one result */
	while(!isclosecap(cs->cap)) {
		updatecache(cs, idx);
...
static int pushcapture(CapState* cs) {
	...
	switch(captype(cs->cap)) {
		...
		case Csimple: {
			...
			if(k > 1) lua_insert(cs->L, -k); /* whole match is first result */
			return k;
		}
		case Cruntime: {
			int n = 0;
...
static int matchl(lua_State* L) {
	...
	size_t i = (ii > 0) ? (((size_t)ii <= l) ? (size_t)ii - 1 : l)
...
static struct luaL_Reg pattreg[] = { { "match", matchl },      { "print", printpat_l },
	                                 { "locale", locale_l },   { "setmaxstack", setmax },
	                                 { "B", pattbehind },      { "C", capture_l },
	                                 { "Cf", fold_l },         { "Cc", capconst_l },
	                                 { "Cg", group_l },        { "Cp", position_l },
	                                 { "Cb", backref_l },      { "Carg", argcap_l },
	                                 { "Cmt", matchtime_l },   { "Cs", capsubst_l },
	                                 { "Ct", tcapture_l },     { "P", pattern_l },
	                                 { "R", range_l },         { "S", set_l },
	                                 { "V", nter_l },          { "type", type_l },
	                                 { "version", version_l }, { NULL, NULL } };

static struct luaL_Reg metapattreg[] = { { "__add", union_l },    { "__pow", star_l },
	                                     { "__sub", diff_l },     { "__mul", concat_l },
	                                     { "__div", rcapture_l }, { "__unm", unm_l },
	                                     { "__len", pattand_l },  { NULL, NULL } };
